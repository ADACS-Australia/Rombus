...
class ReducedBasis:
  ...
  def proj_error_from_basis(self, basis, h):
    ...
    dim = len(basis[:,0])
    return norm**2-np.sum(np.abs(self.alpha1(basis[ii], h))**2 for ii in range(dim))
  ...
  def proj_errors_from_basis(self, basis, hs):
    ...
    #dim = len(basis[:,0])
    #return [norms**2-np.sum(np.abs(self.alpha1(basis[ii], hh))**2 for ii in range(dim)) for hh in hs]
  ...
  def proj_mismatch_from_basis(self, basis, h):
    ...
    dim = len(basis[:,0])
    return 1.-np.real(np.sum(np.abs(self.alpha1(basis[ii], h))**2 for ii in range(dim)))/norms
  ...
  def projection_from_alpha(self, basis, alpha):
    """Project a function h onto the basis functions in terms of pre-computed alpha matrix entry"""
