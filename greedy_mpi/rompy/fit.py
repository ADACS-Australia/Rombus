...
class Spline1d:
	...
	def fit(self, p, der=0):
		if len(self.dim) == 1:
			...
			return np.array([splev(p, [self.knots, self.coeffs[ii], self.degree]) for ii in range(self.dim[0])])
...
class Polynomial:
	...
	def __init__(self, x, y, deg=3):
		...
		ref = lib.map_intervals(x, -1., 1.)
		
		self.shape = np.shape(y)
		if len(self.shape) == 1:
			self.fitparams_ref = np.polyfit(ref, y, deg=deg)
			self.fitparams = self.map_poly_coeffs(self.fitparams_ref, ref, np.min(x), np.max(x))
		elif len(self.shape) > 1:
			if len(np.shape(deg)) == 0:
				self.fitparams_ref = [np.polyfit(ref, y[ii], deg=deg) for ii in range(self.shape[0])]
			else:
				self.fitparams_ref = [np.polyfit(ref, y[ii], deg=deg[ii]) for ii in range(self.shape[0])]
			self.fitparams = [self.map_poly_coeffs(pp, ref, np.min(x), np.max(x)) for pp in self.fitparams_ref]
		else:
			raise Exception( "Expected a non-empty array.")
		pass
	...
	def map_poly_coeffs(self, coeffs, x, a, b):
		...
		return mapped_coeffs[::-1]	# Reverse order of polynomial coefficients for use with np.polyval
	...
	def fit(self, p):
		if len(self.shape) == 1:
			...
			return np.array([np.polyval(self.fitparams[ii], p) for ii in range(self.shape[0])])
...
class UserDefined:
	...
	def __init__(self, x, y, fn, p0=None, maxfev=1000):
		...
		if len(self.shape) == 1:
			...
			self.fitparams = [curve_fit(fn, x, y[ii], p0=p0, maxfev=maxfev)[0] for ii in range(self.shape[0])]
		else:
			raise Exception( "Expected a non-empty array.")
		pass
	...
	def fit(self, p):
		if len(self.shape) == 1:
			...
			return np.array([self.fn(p, *self.fitparams[ii]) for ii in range(self.shape[0])])
