	...
	def __call__(self, args, quad_type):#, multidomainQ=False):
		...
		if type(num_or_rate) == list:
			if len(num_or_rate) != dim[0]-1:
				raise Exception( "Expecting equal number of intervals and number of samples/sample rates.")
		
		# Check how many quadrature rules are specified
		if type(quad_type) == list:
			type_quad_type = 'list'
			if len(quad_type) != len(num_or_rate):
				raise Exception( "Expecting equal number of intervals and specified quadrature rules.")
		else:
			type_quad_type = 'str'
		...
		if len(dim) == 1:
			...
			if dim[0] > 2:
				if type_quad_type == 'str':
					ans = np.hstack([self.switch_dict[quad_type](args[ii], args[ii+1], num_or_rate[ii]) for ii in range(dim[0]-1)])
				elif type_quad_type == 'list':
					ans = np.hstack([self.switch_dict[quad_type[ii]](args[ii], args[ii+1], num_or_rate[ii]) for ii in range(dim[0]-1)])
			...
			ans = [self.tuple_to_vstack(self.meshgrid2(*nodes)).T, np.array([np.prod(weights_nd[ii]) for ii in range(len(weights_nd[:,0]))])]
...
class Real:
	...
	def dot(self, h1, h2):
		...
		if len(dim) == 1:
			...
			return np.dot(self.weights, np.sum( h1real[ii]*h2real[ii] for ii in range(dim[0])))
...
class Complex:
	...
	def dot(self, h1, h2):
		...
		if len(dim) == 1:
			...
			return self.integral(np.sum(np.conj(h1[ii])*h2[ii] for ii in range(dim[0])))
...
class InnerProduct(Real, Complex):
	...
	def Ln(self, h, n):
		...
		# FIXME: h cannot be a multi-dim array
		if n > 0:
			return (np.dot(self.weights, np.abs(h)**n))**(1./n)
		pass
